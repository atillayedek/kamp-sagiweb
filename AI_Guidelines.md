...
3. **Yapılandırılmış çıktı:** `output_config.format` + `betaZodOutputFormat(schema)` (`@anthropic-ai/sdk/helpers/beta/zod`), `client.beta.messages.create()` ile (sunucu tarafı yedek model yalnızca beta API'de; D-043). Yanıtın **son** metin bloğu sunucuda `JSON.parse` + Zod ile doğrulanır, ardından normalize edilip `contracts` şemasından geçer (D-044); izinli enum dışı değer reddedilir. Eski `output_format` parametresi kullanılmaz.
...
8. **Timeout / retry:** TypeScript SDK'da `timeout` **milisaniye**; projede `timeout: 45_000`, `maxRetries: 1`. Toplam süre `timeout × (maxRetries+1)`'e ulaşabileceği için callable süresi (`callables.<ad>.timeoutSeconds`, istemci +10 sn) buna göre ayarlanır. Uygulama katmanında yalnızca şema uyuşmazlığında **en fazla 1** yeniden deneme (D-048).
...
13. **Maliyet:** Kullanıcı başına günlük kota ve toplam günlük token tavanı Firebase params'ta; tavan parçalı sayaçla transaction içinde ayrılır ve gerçek `usage` ile mutabakat yapılır (D-047). Aşımda kullanıcıya anlaşılır mesaj + elle doldurma yolu. `usage` alanları (PII içermeden) loglanır.
