...
| Kullanım sayaçları | Kullanıcı kimliği (belge adında), günlük taslak/ayrıştırma/yayın ve kulüp/etkinlik/bildirim sayıları; kimliksiz günlük token toplamı (`rateLimits`); kimliksiz sayaç olay işaretleri (`counterEvents`) | Kötüye kullanım ve maliyet sınırı | BELİRSİZ | 48 saat (`expiresAt`) — ÖNERİ | Firebase (Google); istemciye kapalı | Süre dolunca silinir |
| İlgi bildirimi | İlana "İlgileniyorum" diyen kullanıcının kimliği ve zamanı (`needs/{id}/interests`), ilan sahibine bildirim (`fromUid`) | İlan sahibinin ilgilenenleri görmesi | BELİRSİZ (kullanıcının kendi eylemi) | İlgi geri alınana veya hesap silinene kadar; geri alınınca bildirim de silinir | Firebase (Google); ilan sahibi aynı kampüsteyse adı/bölümü görür | Geri alma her zaman mümkün; hesap silmede silinir (Faz 11) |
| Kaydedilen ilanlar | Kullanıcının kaydettiği ilan kimlikleri (`savedNeeds/{uid}`) | Kişisel liste | BELİRSİZ | Kullanıcı silene veya hesap silinene kadar | Firebase (Google); yalnızca kullanıcı | Hesap silmede silinir (Faz 11) |
| Topluluk içeriği | Gönderi metni, yorum metni (yazar kimliği ve üniversitesiyle), beğeniler (kullanıcı kimliği), kulüp/etkinlik bilgileri, kulüp üyelikleri ve etkinlik katılımları (kullanıcı kimliği + zaman) | Kampüs akışı, kulüp ve etkinlik işlevleri | BELİRSİZ | Kullanıcı silene (gönderi/yorum/beğeni/üyelik/katılım) veya hesap silinene kadar; kulüp ve etkinlik silme akışı Faz 12 | Firebase (Google); görünürlüğe göre aynı üniversitedeki veya tüm doğrulanmış öğrenciler; beğeni/üye/katılımcı listeleri içeriği görebilenlere açık | Kendi gönderisini (yorum ve beğenileriyle) ve yorumunu silebilir; hesap silmede kapsam S-18 |
| İçerik bildirimi (rapor) | Bildiren kimliği ve üniversitesi, hedef içerik yolu ve sahibinin kimliği, sebep, açıklama (kişisel bilgi maskelenir), **bildirim anındaki içeriğin kopyası** (maskesiz — kişisel bilgi paylaşımı bildirimlerinin incelenebilmesi için) | Moderasyon, platform güvenliği | BELİRSİZ | BELİRSİZ — ÖNERİ: karardan sonra 1 yıl (S-35) | Firebase (Google); yalnızca moderatörler; içerik sahibine bildirenin kimliği gösterilmez | İçerik silinse de kopya rapor kapanana kadar kalır; saklama kararı Faz 12 |
| Üniversite listesi | Kurum adı ve il (kişisel veri değil). Kaynak dosyadaki rektör adı, telefon, e-posta ve adres **alınmaz** | Kampüs seçimi ve izolasyon | — | Süresiz (referans verisi) | Firebase (Google); oturum açmış herkes okur | — |
