...
export function TearOffStrip({ actions, context, className }: TearOffStripProps) {
  ...
  function select(action: TearOffAction) {
    const next = action.toggle ? !pressed[action.id] : true;
    if (action.toggle) setPressed((current) => ({ ...current, [action.id]: next }));
    ...
    <div className={cn("relative", className)}>
      ...
      <ul className="grid auto-cols-fr grid-flow-col">
        {actions.map((action, index) => {
          const isPressed = Boolean(pressed[action.id]);
          return (
            <li key={action.id} className={cn(index > 0 && "border-l border-dashed border-line-strong/60")}>
              <button
                type="button"
                aria-pressed={action.toggle ? isPressed : undefined}
              ...
              >
                <Icon name={isPressed ? "check" : action.icon} className="size-5 text-primary" />
                <span>
                  {action.label}
